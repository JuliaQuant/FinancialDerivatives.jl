...
# --- Equity options ---
eu_md   = EquityMarketData(spot_price=100.0, risk_free_rate=0.05, dividend_yield=0.01, volatility=0.3)
eu_put  = EuropeanOption(100.0, 90.0, 0.05, 0.01, 0.3, 180 / 365, false)
eu_call = EuropeanOption(100.0, 90.0, 0.05, 0.01, 0.3, 180 / 365, true)

am_md   = EquityMarketData(spot_price=100.0, risk_free_rate=0.05, dividend_yield=0.0, volatility=0.3)
am_put  = AmericanOption(100.0, 90.0, 0.05, 0.3, 180 / 365, false)
am_call = AmericanOption(100.0, 90.0, 0.05, 0.3, 180 / 365, true)

# --- FX options ---
fx_md = FXMarketData(spot_price=100.0, domestic_rate=0.05, foreign_rate=0.025, volatility=0.3)
...
@testset "Black-Karasinski" begin
    @test price(BinomialEngine(steps=2), ird, BlackKarasinski(), ir_md) ≈ [0.2, 0.2, 0.2, 0.2] atol = 0.25
end

@testset "Brennan-Schwartz" begin
    @test price(BinomialEngine(steps=2), ird, BrennanSchwartz(), ir_md) ≈ [0.2, 0.2, 0.2, 0.2] atol = 0.25
end
...
@testset "Rendleman-Bartter" begin
    ...
    @test price(BinomialEngine(steps=2), ird, RendlemanBartter(), ir_md) ≈ [0.2, 0.2, 0.2, 0.2] atol = 0.025
end
